...
# the configuration for the EC2 instance
# for the bastion host
resource "aws_instance" "bastion_host" {
  ami                         = data.aws_ami.latest_amazon_linux.id
  instance_type               = "t2.micro"                                  # Use the t3.nano instance type
  ...
  subnet_id                   = var.public_subnet_a_id   # Use the public subnet for AZ A
  vpc_security_group_ids      = [var.bastion_host_sg_id] # Use the security group you created for the bastion host
